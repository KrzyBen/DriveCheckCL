...
// TODO: reemplazar por un fetch real cuando exista GET /admin/reportes/infracciones
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span className="badge badge-amber">{reporte.estado}</span>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 16 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <video
            ...
            src={`/storage/${reporte.videos[clipActivo]?.path}`}
            controls
            style={{ width: '100%', borderRadius: 12, background: '#111' }}
          />
          <div style={{ display: 'flex', gap: 8 }}>
            {reporte.videos.map((v, idx) => (
              <button key={v.id} className="btn" style={{ flex: 1, borderColor: idx === clipActivo ? 'var(--cl-blue)' : 'var(--border)' }} onClick={() => setClipActivo(idx)}>
                Clip {idx + 1}
              </button>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div className="card" style={{ padding: '1rem 1.25rem', background: 'var(--surface-1)' }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
              ...
              <button className="btn" onClick={agregarInfraccion}>+ Agregar</button>
            </div>
            {mostrarManual && (
              <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {infracciones.map((inf, idx) => (
                <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 10px', background: '#fff', borderRadius: 8, fontSize: 13 }}>
                  <span>{inf.texto}</span>
                  <button className="btn btn-icon" onClick={() => quitarInfraccion(idx)}>✕</button>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-danger-outline" style={{ flex: 1 }} onClick={() => setShowRechazar(true)}>Rechazar</button>
            <button className="btn btn-primary" style={{ flex: 1 }} onClick={onValidar}>Validar y generar PDF</button>
          </div>
        </div>
      </div>
